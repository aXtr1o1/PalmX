...
export default function DataSources() {
    ...
    return (
        <div className="min-h-screen bg-white text-foreground font-sans selection:bg-black selection:text-white">
            ...
            {/* Header */}
            <header className="fixed top-0 w-full bg-white/95 backdrop-blur-md z-40 border-b border-gray-100 px-6 md:px-12 h-[88px] grid grid-cols-3 items-center">
                ...
                <div className="flex flex-col items-center justify-center justify-self-center">
                    <div className="flex items-center gap-3 mb-1">
                        <img src="/brand/palmHills-BlockLogo.png" alt="Palm Hills" className="h-8 w-auto" />
                        <span className="font-serif text-lg tracking-[0.2em] font-bold text-black">PALM HILLS</span>
                    </div>
                    <span className="text-[9px] uppercase tracking-[0.3em] text-[#D22048] font-bold">Data Transparency</span>
                </div>
            ...
            <main className="pt-40 px-6 md:px-12 pb-24 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="text-center mb-20 space-y-6">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-surface rounded-full mb-4 border border-gray-100">
                        <Database size={12} className="text-[#D22048]" />
                        <span className="text-[10px] uppercase tracking-[0.2em] font-medium text-gray-500">System Source of Truth</span>
                    </div>
                    <h1 className="font-serif text-5xl md:text-6xl text-black leading-tight">Verified Intelligence<span className="text-[#D22048]">.</span></h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
                    {verifiedPortfolios.map((portfolio, idx) => (
                        <div key={idx} className="bg-surface/30 p-8 rounded-3xl border border-gray-100 hover:border-[#D22048]/20 transition-all duration-500 group hover:shadow-lg hover:shadow-[#D22048]/5">
                            <h3 className="font-serif text-2xl text-[#D22048] mb-6 px-4 border-l-2 border-[#D22048]">{portfolio.region}</h3>
                            <ul className="space-y-4">
                                {portfolio.projects.map((project) => (
                                    <li key={project} className="flex items-center gap-3 px-4 py-3 bg-white rounded-xl shadow-sm border border-gray-50 group-hover:translate-x-1 transition-transform duration-300">
                                        <CheckCircle2 size={16} className="text-[#D22048] flex-shrink-0" />
